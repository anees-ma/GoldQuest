...
$qry=mysqli_query($con,"insert into clent_details(client_uid,client_id,company_name,address,mobile,email,email2,email3,email4,contact_person,role,gst,service_name,service_price,status,dsagre,dsagreexp,dsagreepero,agrefile,state,date,tatd) 
values('$cmpuid','$client_id','$cmpnme','$addrs','$mobile','$email','$email2','$email3','$email4','$contper','$role','$gst','$servnm','$serpric','$sta_t','$dsagr','$dsagrexp','$agrper','$imgN','$state','$dap','$tatd')");
if($qry){
    ...
    if($log){
        ...
        /*header('location:https://admin.goldquestglobal.in?redi=cmsad');*/
        ?>
       <script>
      /*alert('success');*/
        window.location.assign("https://admin.goldquestglobal.in?redi=cmsad");
  </script>
  <?php
    }
}
