...
if($frnme == 'cdb'){ 
    ...
    /*---For getting services selected by the client only---*/
    $servi_name_cdb=substr($clientres['service_name'], 0, -1);
    $servi_nameexp_cdb=explode(',',$servi_name_cdb);

    $servi_nameexp_cdb=array_map("getClientServicelistSelected",$servi_nameexp_cdb);
    /*------------------------------------------------------*/
?>
<form action="submitcdb.php" method="post" enctype="multipart/form-data" name="submitcdb" id="submitcdb"><!--Candidate Dropbox -->
    ...
    <div class="container" style="margin-top: 25px;">
        <div class="row">
          <div class="col-lg-12">
            <?php 
              ...
              <div class="alert alert-success alert-dismissible">
                <button id="" type="button" class="close" data-dismiss="alert">&times;</button>
          ...
          <div class="col-lg-12">
            <div class="form-title page-title">Create Application</div><!--Candidate dropbox-->
        </div>
<div class="col-lg-6">
  ...
  <input type="hidden" id="clientids_cdb" name="clientid_cdb" value="<?php echo $client_id?>">
      ...
      <table style="border: 1px solid grey;" class="service-table">
      ...
      while($chkres=mysqli_fetch_assoc($chk)){
          ...
          <!-- <tr> -->
              ...
              for($k=0; $k < $subservcnt; $k++){
                if(in_array($subservexp[$k],$servi_nameexp_cdb)){
                  ?>
                  <tr>
                      <td style="text-align: left"><input type="checkbox" name="services[]" value="<?php echo strtolower($subservexp[$k]);?>" id="<?php echo strtolower($subservexp[$k]);?>">&nbsp;<label for="<?php echo strtolower($subservexp[$k]);?>"><?php echo strtoupper(getServiceDisplayName($subservexp[$k],$con));?></label>
                      <?php
    ...
    <form action="createapp.php" method="post" enctype="multipart/form-data" id="creatappsubf">
      <div class="container" style="margin-top: 25px;">
          ...
          if($adclsucc != NULL){
              ?>
          <div class="alert alert-success alert-dismissible">
          <!-- <button id="pusurl" type="button" class="close" data-dismiss="alert">&times;</button> #CH on April 13, 2021 by Anees MA with below line-->
          <button id="" type="button" class="close" data-dismiss="alert">&times;</button>
          <strong>Success!</strong> Application Done Successfully.
          </div>
              <?php
          ...
          <div class="col-lg-12">
              <div class="form-title page-title">Create Application</div><!--Client dropbox-->
          </div>
          ...
          <div class="col-lg-6">
            ...
            <div class="col-lg-6">
              ...
              while($chkres=mysqli_fetch_assoc($chk)){
                  ...
                  // $subservcnt=count($subservexp)-1;#CH - added below line instead
                  //$subservcnt = ($subservid==43)?1:count($subservexp)-1;//For Insta drug test by Anees on Mar 12, 2021--NOTE: Disabled this line on Mar 29 last item is not listing by adding below code
                  ...
                  <!-- <tr> -->
                      ...
                      for($k=0; $k < $subservcnt; $k++){
                        if(in_array($subservexp[$k],$servi_nameexp)){
                          ...
                          ?>
                          <tr>
                              <td style="text-align:left"><input type="checkbox" name="services[]" value="<?php echo $subservid.'#'.strtolower($subservexp[$k]);?>" id="<?php echo strtolower($subservexp[$k]);?>">&nbsp;<label for="<?php echo strtolower($subservexp[$k]);?>"><?php echo strtoupper(getServiceDisplayName($subservexp[$k],$con));?></label>
                                <?php
                              if($subservexp[$k]=='post-graduation'){ echo ': ';?>
                                <select id="pg-dropdown" name="pg_stream" onchange="pgUgChange(this.id)"> 
                                    <option></option>
          ...
          <div class="col-lg-12">
            <button type="submit" class="btn btn-primary" id="add_client" style="width: 20%; margin-left: 30%; margin-right: 30%">Submit</button>
          </div>      
        </div>
      </div>
  </form>
<!--drop box applications few st-->
  ...
  <div  class="task_tabl">
              <table class="dataTable-gn tbl_data tbl_data2" id="appl-entry">
                <thead>
                  <tr>
                  <th>Sl No.</th>
                  ...
                  while($aplselres=mysqli_fetch_assoc($aplsel)){
                      ...
                      <tr>
                          ...
                          <td>
                              ...
                              for($y=0; $y < $docnt; $y++){
                                  ?>
                                  <!-- <a href="<?php echo APP_PATH;?>/<?php echo $docsexp[$y];?>" class="docstyl" target="_blank"></a>&nbsp; #CH-->
                                  <a href="<?php echo APP_PATH;?>/<?php echo $docsexp[$y];?>" class="mt-3" target="_blank"><button class="btn btn-primary">Doc&nbsp;<?php echo $h;?></button></a>&nbsp;
                                  <?php
                                  $h++;
                              }
                              ?>
                          ...
                          <td>
                              <button class="btn btn-primary" onclick="editdrop(this)" data-drlogid="<?php echo $log_id;?>" data-edaplid="<?php echo $aplselres['ref_id'];?>">Edit</button>
...
elseif($frnme == 'apmng'){
    ...
    <div class="task_tabl" style="overflow-x: scroll; clear: both;">
                <table class="tbl_data tbl_data2 dataTable-gn">
                  <thead>
                    <tr>
                    <th>SL No</th>
                    <th>Application ID</th>
                    ...
                    for($d=0; $d < $servtcnt; $d++){
                        ...
                        $serfnnme=$servhas[1];
                        ?>
                        <th class="servclr"><?php echo strtoupper(getServiceDisplayName($serfnnme,$con));?></th>
                        <?php
                    }
                    ?>
                ...
                <tbody>
                    ...
                    <?php 
                    $a=1;
                    //$aplsel=mysqli_query($con,"select * from applications where client_id='$log_id' && status='completed' order by id DESC");#CH bY Anees and replaced with below line on April 7 2021 for getting all reports other than completed

                    $aplsel=mysqli_query($con,"select * from applications where client_id='$log_id' order by id DESC");
                    while($aplselres=mysqli_fetch_assoc($aplsel)){
                        ...
                        <tr>
                            ...
                            <td class="seppad">
                                ...
                                if($over_all_sta == 'completed'){
                                  ?>
                                  <a href="<?php echo APP_PATH;?>/pdf/rpt.php?reff_id=<?php echo $appic_id;?>" class="" target="_blank"><button class="btn btn-primary">REPORT</button></a>
                                  <?php
                                }
                                else{
                                    echo 'NOT READY';
...
elseif($frnme == 'support'){
    ...
    <div class="task_tabl">
        ...
        <ul style="list-style-type: none;">
            <li><i class="fa fa-arrow-right support-i" aria-hidden="true"></i>Indian Scope of Services</li>
            <li><i class="fa fa-arrow-right support-i" aria-hidden="true"></i>Change in Scope of Service / Additional Services</li>
            <li><i class="fa fa-arrow-right support-i" aria-hidden="true"></i>Esclation Matrix</li>
            <li><i class="fa fa-arrow-right support-i" aria-hidden="true"></i>24/7 Support</li>
            <li><i class="fa fa-arrow-right support-i" aria-hidden="true"></i>Any Insufficiency</li>
            <li><i class="fa fa-arrow-right support-i" aria-hidden="true"></i>Single Point of Contacts</li>
            <li><i class="fa fa-arrow-right support-i" aria-hidden="true"></i>International Scope of Services</li>
        </ul>
        We request you to call <strong>Key Accounts Manager Mrs. Aarti Mohan (7904847400)</strong> from Monday to Saturday 9.30 AM to 6.30 PM, or <strong>Manjunath (9945891310)</strong> Head Business Excellence 24/7.
        </div><br>
                <table class="tbl_data tbl_data_cpy" style="border: none;border-collapse: separate;">
                    <!--<th>SL No</th>-->
                    <th style="border: none">Level</th>
                    <th style="border: none">Relations</th>
                        <tr>
                            <td>1st Level</td>
                            <td class="sprsty" style="text-align: center"><span style="color:#3d75a4">Mrs. Aarti Mohan</span></td>
                        </tr>
                        <tr>
                            <td>SPOC</td>
                            <td class="sprsty" style="text-align: center">Key Accounts Manager
                        ...
                        <tr>
                            <td>2nd Level</td>
                            <td  class="sprsty" style="text-align: center"><span style="color:#3d75a4">Mr. Manjunath H S</span>
                        ...
                        <tr>
                            <td>3rd Level</td>
                            <td  class="sprsty" style="text-align: center"><span style="color:#3d75a4">Mr. Jaykumar</span>
                            <br>Chairman
