...
if($selcnt > 0){
   ...
   // print_r($ref_idexp);
    ...
    $ref_id=$ref_idexp[0].'-'.$ref_idexp[1].'-'.$refplus;
}
else{
    $ref_idcr=$client_uid;
    /*$ref_idexp=explode('-',$ref_idcr);*/
...
foreach($_FILES["mudcs"]["tmp_name"] as $key=>$tmp_name) {
             ...
             $coverImgdir = APP_FILE_PATH.'assets/'.$mobile.'/'.'applications/';
            if(!is_dir($coverImgdir)){
                mkdir($coverImgdir,0777,true);
            }
              $imgRANDFS=$imgRANDM.'-'.date('d-m-y').'-'.time().'.'.$ext;
              ...
              /*if($_FILES["uoida"]["tmp_name"]){
               ...
               $coverImgdir_uoida = APP_FILE_PATH.'assets/'.$mobile.'/'.'applications/Insufficiency-document-attachment/';
              if(!is_dir($coverImgdir_uoida)){
                  mkdir($coverImgdir_uoida,0777,true);
              }
                $imgRANDFS_uoida=$imgRANDM_uoida.'-'.date('d-m-y').'-'.time().'.'.$ext_uoida;
                $target_uoida = "$coverImgdir_uoida".$imgRANDFS_uoida;
                move_uploaded_file($source_uoida, $target_uoida);
                

                $imagepath_uoida = $imgRANDFS_uoida;
                $imgN_uoida='assets/'.$mobile.'/applications/Insufficiency-document-attachment/'.$imagepath_uoida;
              }*/
...
'$national_id_check_1','$national_id_check_2','$company_site_visit','$credit_verification','$ug_stream','$pg_stream','$services','$rm_mr')");
...
if($qry){
 include 'adminmail.php'
    ?>
       <script>
      /*alert('success');*/
        window.location.assign("<?php echo APP_PATH;?>/customer-index.php?suBD=cretapl");
  </script>
  <?php
}else{
