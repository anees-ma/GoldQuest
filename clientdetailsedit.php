...
?>
 ...
 <form action="editclientdetails.php" method="post" enctype="multipart/form-data" style="text-align:left" id="adcl_submi">
  ...
  <?php 
      $dateOfServiceAgreement = ($seragstd!='')?date('d-m-Y',strtotime($seragstd)):$seragstd;
  ?>
  <div class="form-group">
    <label for="dsagr">Date of Service Agreement: <span style="color:red">*</span></label>
    <input type="text" class="form-control datepicker" name="dsagr" id="dsagr" max="9999-12-31" value="<?php echo $dateOfServiceAgreement;?>">
  </div>
  <div class="form-group">
  ...
  <div class="form-group">
    <label for="is_custom_temp_e">Required Custom Template: </label>
    <select style="float: right; width: 250px;" id="is_custom_temp_e" name="is_custom_temp">
        <option value="no" <?php if($clientres['is_custom_temp']=='no'){ echo 'selected'; }?>>No</option>
        <option value="yes" <?php if($clientres['is_custom_temp']=='yes'){ echo 'selected'; }?>>Yes</option>
    </select>
  </div>
  <div class="form-group custom_temp_e" id="custom_logo_div_e" style="<?php if($clientres['is_custom_temp']=='no'){ echo 'display: none'; }?>">
    <label>Upload Custom Logo:</label><br>
    <input type="file" class="form-control" name="custom_logo" id="custom_logo">
  </div>

  <div class="form-group custom_temp_e" id="custom_address_div" style="<?php if($clientres['is_custom_temp']=='no'){ echo 'display: none'; }?>">
    <label>Custom address:</label><br>
    <textarea class="form-control" name="custom_address" id="custom_address"><?php echo $clientres['custom_address']?></textarea>
  </div>
  <div class="form-group">
    ...
    for($i=0; $i<$servcnt; $i++){
        ...
        ?>
        <!-- <input type="text" name="servck" id="servck" value="<?php echo strtoupper($servi_nameexp[$i]);?>" style="width:45%" disabled>&nbsp;&nbsp; #CH-->
        <!-- <input type="text" name="servck" id="servck" value="<?php echo strtoupper(explode('#',$servi_nameexp[$i])[1]);?>" style="width:45%" disabled>&nbsp;&nbsp; -->
        <input type="text" name="servck" id="servck" value="<?php echo strtoupper(getServiceDisplayName(explode('#',$servi_nameexp[$i])[1],$con)) ?>" style="width:45%" disabled>&nbsp;&nbsp;
  ...
  <!--add extra services st-->
    ...
    <div class="form-group">
        <lable class="mb-5 pb-5">Add Extra Services:</lable>
      <table class="editClient" style="border: 1px solid grey; width: 84%">
          <th style="padding: 2px;background-color: #3e76a5; color: white; border-right: 2px solid white">Service Name</th>
...
<script>
    ...
    $('#add_client').click(function(){
        ...
        let validation_logo = 1;
        var is_custom_temp = $("#is_custom_temp_e").val();
        ...
        if(vcomp_nme != '' && vcref_id != '' && vaddr_v != '' && vstate != '' && vmobile != '' && vemail != '' && vagperi != '' && vdatagst != '' && validation_logo!=0){
            /*$('.bx_1').show(1000);*/
...
<script>
  $(document).on('change','#is_custom_temp_e',function(){

  let val = $('#is_custom_temp_e').val();
  if(val=='yes'){
    $('.custom_temp_e').show();
  }else{
    $('.custom_temp_e').hide();
